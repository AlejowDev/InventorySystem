...
const User = {
    create: (userData, callback) => {
        ...
        const query = "INSERT INTO users (document, name, username, password, role, isTemporaryPassword) VALUES (?, ?, ?, ?, ?, ?)";
        db.query(query, [userData.document, userData.name, userData.username, userData.password, userData.role, userData.isTemporaryPassword], callback);
    },
    ...
    updatePassword: (document, hashedPassword, callback) => {
        ...
        const query = "UPDATE users SET password = ?, isTemporaryPassword = 0 WHERE document = ?";
